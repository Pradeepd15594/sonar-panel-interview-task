...
export class NumbersOnly {
    ...
    @HostListener('keydown', ['$event']) onKeyDown(e: KeyboardEvent) {
        ...
        /**@when minus sign is allowed, add its key to allowed key only when the cursor is in the first position, and first character is different from decimal separator */
    ...
    validateValue(value: string): void {
        ...
        // when a numbers begins with a decimal separator, fix it adding a zero in the beginning
        let firstCharacter = value.charAt(0);
        if (firstCharacter == this.decimalSeparator)
            value = 0 + value;

        // when a numbers ends with a decimal separator, fix it adding a zero in the end
        let lastCharacter = value.charAt(value.length-1);
        if (lastCharacter == this.decimalSeparator)
            value = value + 0;

        // test number with regular expression, when number is invalid, replace it with a zero
